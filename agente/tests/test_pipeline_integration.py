...
def test_run_pipeline_with_details_mock_contract(monkeypatch) -> None:
    ...
    assert all("llm_decision_source" in r.evidence for r in result.recommendations)
    assert all(r.evidence.get("llm_decision_source") == "azure_openai" for r in result.recommendations)
    ...
    assert all("llm_decision_source" in record.evidence for record in result.recommendation_records)
...
def test_run_pipeline_marks_market_status_for_real_mode(monkeypatch) -> None:
    monkeypatch.setattr(LangChainInvestmentAgent, "_llm_decide_recommendation", _stub_llm_decide)
    monkeypatch.setattr(LangChainInvestmentAgent, "_llm_reflection", _stub_llm_reflection)
    ...
    assert result.ticker_statuses[0].market_status == "ok_real"
    assert result.recommendation_records[0].market_status == "ok_real"
...
def test_run_pipeline_recommendation_rationale_is_descriptive(monkeypatch) -> None:
    monkeypatch.setattr(LangChainInvestmentAgent, "_llm_decide_recommendation", _stub_llm_decide)
    monkeypatch.setattr(LangChainInvestmentAgent, "_llm_reflection", _stub_llm_reflection)
...
def test_run_pipeline_builds_structured_recommendation_record(monkeypatch) -> None:
    monkeypatch.setattr(LangChainInvestmentAgent, "_llm_decide_recommendation", _stub_llm_decide)
    monkeypatch.setattr(LangChainInvestmentAgent, "_llm_reflection", _stub_llm_reflection)
    ...
    assert "llm_decision_source" in record.evidence
